...
import pro.sky.AnimalShelter.handlers.adoptionMenuHandlers.DogTrainerAdviceHandler;
...
public class DogTrainerAdviceHandlerTest {
    ...
    public void setUp() {
        initMocks(this);
        dogTrainerAdviceHandler = new DogTrainerAdviceHandler(chatStateService, telegramBot, commonUtils);
    ...
    public void testDogTrainerAdviceHandler() {
        Long chatId = 123L;
        BotCommand currentState = BotCommand.DOG_TRAINER_ADVICE;

        when(chatStateService.getCurrentStateByChatId(chatId)).thenReturn(currentState);
        SendMessage message = new SendMessage(chatId.toString(), DOG_TRAINER_ADVICE_TEXT);
